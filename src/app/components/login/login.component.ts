...
export class LoginComponent implements OnInit {
  ...
  loginForm:FormGroup;
  constructor(private formBulder:FormBuilder, private loginService:LoginService, private userService:UserService, private router:Router) { }
  ...
  loginhandler(){
    ...
    this.loginService.checkLogin(userDto).subscribe({
      next:response=>{
        if(response!=null){
          Swal.fire( 'Logged in successfully! User Name:',`${response.fullName}`,'success');
